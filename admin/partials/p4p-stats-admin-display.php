 ...
 *
 * @package    P4p_Stats
 * @subpackage P4p_Stats/admin/partials
 */
?>
...
<div class="wrap">
    ...
    <form method="post" name="p4p_data">
        <?php 
		
			if ( isset( $_REQUEST['submit_data'] ) ) {
				do_action('show_p4p_data');
			} 
		
			settings_fields($this->plugin_name); 
			__( submit_button('Get P4P Data', 'primary', 'submit_data', TRUE)); 
		?>    
    </form>
    ...
    <h2 class="nav-tab-wrapper"><?php _e('Login customization', $this->plugin_name);?></h2>
        ...
        <!-- add your logo to login -->
            <fieldset>
                ...
                <div id="upload_logo_preview" class="p4p_stats-upload-preview <?php if(empty($login_logo_id)) echo 'hidden'?>">
                    <img src="<?php echo $login_logo_url; ?>" />
                    <button id="p4p_stats-delete_logo_button" class="p4p_stats-delete-image">X</button>
                </div>
            </fieldset>
        ...
        <?php __( submit_button('Save all changes', 'primary','submit', TRUE)); ?>
